...
<html>
 ...
 background: rgba(255, 255, 255,0.5);
  ...
  <div class="panel-heading">资源详细</div>
  <div class="panel-body">
    ...
    <%
		...
		Query  query2 = session2.createQuery("from Comment");
		Object Object1=query.list();
		Object Object2=query2.list();
		ArrayList<Topic> ss=new ArrayList<Topic>();
		ArrayList<Comment> ss2=new ArrayList<Comment>();
		ss=((ArrayList<Topic>)Object1);
		ss2=((ArrayList<Comment>)Object2);
		...
		int TopicId=Integer.parseInt(TopicIdStr)-1;
		ArrayList <String> ssss=new ArrayList <String>() ;
		ss2.get(1).getTopic().getTopicName();
	...
	<ul class="topic">
		...
		<h3>
			...
			<%
			String id=ss.get(TopicId).getTopicId();
			%>
			·
			<a href="info.jsp?TopicId=<%=id%>" target="_blank"><%=ss.get(TopicId).getTopicName()%></a></span>
			·
		</h3>
		<a href="#" target="_blank"><img src="/e_doc/src/red.jpg" class="article" /> </a>
		<p>
		上传日期：<span class="PublishTime">
		<%
			Date d=ss.get(TopicId).getPublishTime();
			out.print(1900+d.getYear()+"-"+d.getMonth()+"-"+d.getDate()+" "); 
			out.print(d.getHours()+":"+d.getMinutes()); 
			
		%>
		</span> 下载次数：<span class="DownloadNum"><%=ss.get(TopicId).getDownloadNum()%></span>
		</p>
		<p>下载链接：<span class="Link"><%=ss.get(TopicId).getLink()%></span> </p>
		<p>介绍：<span class="Link"><%=ss.get(TopicId).getWordDescription() %></span> </p>
		<p>
			评论:
			<%	
			int commentNum=0;
			...
			{	
				if (ss2.get(i).getTopic().getTopicId().equals(TopicIdStr))
				{
					out.println("</p>第"+ ++commentNum +"楼: "+ss2.get(i).getCommentContent()+"</p>");
				}
			}
			%>
